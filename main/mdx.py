...
class MDX:
    ...
    def __init__(self, model_path: str, params: MDXModel, processor=0):
        ...
        # Progress bar related attributes
        self.prog = None
        self.prog_lock = threading.Lock()
        self.prog_desc = "Processing"
    ...
    def _update_progress(self, n=1, desc=None):
        """Thread-safe progress bar update"""
        with self.prog_lock:
            if self.prog is not None:
                if desc:
                    self.prog.set_description(desc)
                self.prog.update(n)
    ...
    def _process_wave(self, mix_waves, trim, pad, q: queue.Queue, _id: int):
        ...
        with torch.no_grad():
            ...
            for i, mix_wave in enumerate(mix_waves):
                # Update progress with thread info
                self._update_progress(1, f"Processing chunk {_id+1}")
    ...
    def process_wave(self, wave: np.array, mt_threads=1):
        ...
        total_chunks = 0
        for batch in waves:
            n_sample = batch.shape[1]
            trim = self.model.n_fft // 2
            gen_size = self.model.chunk_size - 2 * trim
            pad = gen_size - n_sample % gen_size
            n_chunks = len(range(0, n_sample + pad, gen_size))
            total_chunks += n_chunks

        # Initialize progress bar with proper total and description
        self.prog = tqdm(
            total=total_chunks,
            desc="MDX Processing",
            unit="chunk",
        ...
        self.prog.set_description("Finalizing")
        self.prog.set_postfix_str("Concatenating results...")
        self.prog.close()
...
def run_mdx(model_params, output_dir, model_path, filename, exclude_main=False, exclude_inversion=False, suffix=None, invert_suffix=None, denoise=False, keep_orig=True, m_threads=2, base_device="cuda"):
    ...
    if denoise:
        # Show progress for denoising operation
    ...
    if not exclude_main:
        ...
        print(f"Saved main output: {main_filepath}")
    ...
    if not exclude_inversion:
        ...
        print(f"Saved inverted output: {invert_filepath}")
